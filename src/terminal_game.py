...
from animals import Animal, CreateAttackAbilityTypes, CreateAnimalDeck, CreateEachAnimal
from animal_grid import AnimalSight, CreateEnvironmentGrid, AddAnimalToGrid
import numpy as np
...
def JoinBattleEffects():
    ...
    UniqueAnimals = np.unique(np.array([Animal2.AnimalName for Animal2 in Animal.InBattle], dtype = object))
    for AnimalName in UniqueAnimals:
        AllBattlefieldAnimalsWithThisType = np.array([OtherAnimal for OtherAnimal in Animal.InBattle if OtherAnimal.AnimalName == AnimalName], dtype = object)
        if AllBattlefieldAnimalsWithThisType.shape[0] > 1:
            #Assume Each of The Same Animal Has Grouping or Not
            FoundAbility = Animal.FindAbility("Grouping", AllBattlefieldAnimalsWithThisType[0].AbilityTypes, "None")
...
def StartTurnEffects(Animal, Weather, Environment, NoMovement, SkipTurn):
    ...
    if Animal.CurrentAbilities["ColdBlooded"]:
        FoundAbility = Animal.FindAbility("ColdBlooded", Animal.AbilityTypes, "None")
    ...
    if Animal.CurrentAbilities["Intellect"] and Environment in ("Forest", "Grasslands", "Tundra"):
        FoundAbility = Animal.FindAbility("Intellect", Animal.AbilityTypes, "None")
        FoundAbility[0].AbilityFunction(FoundAbility[0], Animal, Environment)

    #Rations
    if Animal.CurrentAbilities["Rations"]:
        FoundAbility = Animal.FindAbility("Rations", Animal.AbilityTypes, "None")
        FoundAbility[0].AbilityFunction(FoundAbility[0], Animal, FoundAbility[1])
...
def MovementAction(Animal, Environment, DayNight, Grid2D, AttackRadiusAnimals, VisionPlusRadius, ValidMovements, ObstaclesBeside, AnimalsSeen):
    ...
    if Movement.startswith("2x"):
        FoundAbility = Animal.FindAbility("Exhaustion", Animal.AbilityTypes, "None")
        FoundAbility[0].AbilityFunction(FoundAbility[0], Animal, FoundAbility[1])
...
def ExtraAction(Animal, DayNight, Grid2D, AttackRadiusAnimals, VisionPlusRadius, ValidMovements, ObstaclesBeside, AnimalsSeen):
    ...
    Animal.InBattle = np.delete(Animal.InBattle, np.where(Animal.InBattle == Animal)[0][0])
    ...
    if Action == "Rattle":
        ...
        if Animal.CurrentAbilities["Camoflauge"] != 9999:
            FoundAbility = Animal.FindAbility("Camoflauge", Animal.AbilityTypes, Action)
            if FoundAbility is not None:
                FoundAbility[0].AbilityFunction(FoundAbility[0], Animal, FoundAbility[1])

        #Fear (Assuming if Animal Sees Opposing Animals, The Opposing Animals See Animal).
        FoundAbility = Animal.FindAbility("Fear", Animal.AbilityTypes, Action)
        if FoundAbility is not None:
        ...
        for Defender in VisionPlusRadius:
            if Defender in Animal.InBattle:
                Animal.InBattle = np.delete(Animal.InBattle, np.where(Animal.InBattle == Defender)[0][0]) #Remove Chosen Defenders to Not Choose The Same Defender Twice.
        ...
        if Animal.CurrentAbilities["Night Vision"] != 1:
            FoundAbility = Animal.FindAbility("Night Vision", Animal.AbilityTypes, Action)
            if FoundAbility is not None:
                FoundAbility[0].AbilityFunction(FoundAbility[0], Animal, FoundAbility[1])
                '''Determine Who The Animal Can See (Using Camoflauge, Night Vision).'''
                AttackRadiusAnimals, VisionPlusRadius, ValidMovements, ObstaclesBeside, AnimalsSeen = AnimalSight(Animal, Grid2D, DayNight)
            ...
            if ChosenAttackType.upper() != "N":
                ...
                if ValidDefenders.shape[0] != 0:
                    ...
                    if ChosenAttackType.SplashDamage[0] and np.where(Defenders == "None")[0].shape[0] == 0: #Assume Splash Hits Every Seen Opposing Animal in Attack Radius Right Now, Not Through Obstacles or Animals.
                        ...
                        if Defenders[0] in Animal.InBattle:
                            Animal.InBattle = np.delete(Animal.InBattle, np.where(Animal.InBattle == Defenders[0])[0][0]) #Remove Chosen Defenders to Not Choose The Same Defender Twice (Consistency).
...
def EndTurnEffects(Animal, Weather, Environment, Action, Movement, DeathGrid2D):
    ...
    if Animal.CurrentAbilities["Venom"] > 0:
        ...
        FoundAbility = Animal.FindAbility("Venom")
        FoundAbility.AbilityFunction(FoundAbility, Animal, Reverse = True)

    #Paralysis
    if Animal.CurrentAbilities["Paralysis"][0]:
        FoundAbility = Animal.FindAbility("Paralysis")
        FoundAbility.AbilityFunction(FoundAbility, Animal, Reverse = True)

    #Cold Blooded
    if Animal.CurrentAbilities["ColdBlooded"]:
        if Weather == "Blizzard":
            FoundAbility = Animal.FindAbility("ColdBlooded")
    ...
    if Animal.CurrentAbilities["Camoflauge"] != Animal.OriginalCamoflauge:
        ...
        FoundAbility = Animal.FindAbility("Camoflauge")
    ...
    if Animal.CurrentAbilities["Night Vision"] != Animal.OriginalNightVision:
        FoundAbility = Animal.FindAbility("Night Vision")
    ...
    #Flinch
    if Animal.CurrentAbilities["Flinch"]:
        FoundAbility = Animal.FindAbility("Flinch")
        FoundAbility.AbilityFunction(FoundAbility, Animal, Reverse = True)

    #Bleed
    if Animal.CurrentAbilities["Bleed"] > 0:
        FoundAbility = Animal.FindAbility("Bleed")
        if FoundAbility is not None:
            FoundAbility.AbilityFunction(FoundAbility, Animal, Reverse = True)
...
def EndTurn(Animal, SkipTurn, NoMovement, Defenders, Defenders2, DeathGrid2D):
    ...
    if not(SkipTurn):
        ...
        for Animal2 in AllAnimals:
            if Animal2.Health > 0:
                Animal.InBattle = np.append(Animal.InBattle, Animal2)
                ...
                if AllBattlefieldAnimalsWithThisType.shape[0] > 1:
                    for SameAnimal in AllBattlefieldAnimalsWithThisType:
                        FoundAbility = Animal.FindAbility("Grouping", SameAnimal.AbilityTypes, "None")
                        if FoundAbility is not None:
        ...
        if Animal.Health <= 0:
            AllBattlefieldAnimalsWithThisType = np.array([OtherAnimal for OtherAnimal in Animal.InBattle if OtherAnimal.AnimalName == Animal.AnimalName], dtype = object)
            Animal.InBattle = np.delete(Animal.InBattle, np.where(Animal.InBattle == Animal)[0][0])
...
def main():
    ...
    while not(GameOver) and Turn < LastTurnPlus: #Change Turn Count Here
        ...
        Player1Animals = [Animal for Animal in Animal.InBattle if Animal.Player == Player1]
        Player2Animals = [Animal for Animal in Animal.InBattle if Animal.Player == Player2]
        if Turn == 1 or (Player1Animals != [] and Player2Animals != []):
            print(f"\nTurn {Turn}:")

            #Print The Stats of All Animals Starting The Turn.
            Animal.PrintAllAnimalsInBattle()
            ...
            for Animal in np.array(Animal.InBattle, dtype = object):
                SkipTurn = False
                NoMovement = False
